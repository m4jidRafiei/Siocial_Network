...
#         last_next_resource = pd.DataFrame(columns = ['resource'])
#         for index, row in self.snDataFrame.iterrows():
#             if(index > 0 and self.snDataFrame.loc[index,'trace_id'] != self.snDataFrame.loc[index-1,'trace_id']):
#                 last_next_resource = last_next_resource.append({'resource': self.snDataFrame.loc[index-1,'next_resource']},ignore_index=True) 
#         last_next_resource = last_next_resource.append({'resource': self.snDataFrame.loc[index,'next_resource']},ignore_index=True)
#            
#         last_next_activity = pd.DataFrame(columns = ['activity'])
#         for index, row in self.snDataFrame.iterrows():
#             if(index > 0 and self.snDataFrame.loc[index,'trace_id'] != self.snDataFrame.loc[index-1,'trace_id']):
#                 last_next_activity = last_next_activity.append({'activity': self.snDataFrame.loc[index-1,'next_activity']}, ignore_index=True) 
#         last_next_activity = last_next_activity.append({'activity': self.snDataFrame.loc[index,'next_activity']}, ignore_index=True)      
#  
#         allresources = np.concatenate([self.snDataFrame['resource'], last_next_resource['resource']], axis=0)
#         allactivities = np.concatenate([self.snDataFrame['activity'], last_next_activity['activity']], axis=0)
...
#         RscActMatrix_jointactivities_df = pd.DataFrame(RscActMatrix_jointactivities, index=self.resourceList, columns=self.activityList)
#           
#         n_cpus = psutil.cpu_count()
#         
#         if(RscActMatrix_jointactivities_df.shape[0] > n_cpus):
#             each_frame_count = floor(RscActMatrix_jointactivities_df.shape[0] / n_cpus)
#         else:
#             each_frame_count = 1
#  
#         list_df = [RscActMatrix_jointactivities_df[i:i+each_frame_count] for i in range(0,RscActMatrix_jointactivities_df.shape[0],each_frame_count)]
#         
#         manager = mp.Manager()
#         return_list = manager.list()
#         
...
#         for i in range(0,len(list_df)):
#             if(list_df[i].shape[0] > 0):
#                 p = mp.Process(target=SNCreator.compose_worker, args=(self, list_df[i], new_activityList, activity_substitutions,return_list))
#                 p.start()  
#                 jobs.append(p)
#         for proc in jobs:
#             proc.join()       
#             print('joined')       
#         
#         RAmatrix = np.zeros([len(self.resourceList),len(new_activityList)])
#         for i in range(0,len(return_list)):
#             print("==========" + str(i))
#             if(i % 2 == 0):
#                 list_index = return_list[i]
#             else:
#                 count =0
#                 for row in list_index:
#                   #  RAmatrix[row] = return_list[i].iloc[[count]]
#                     for column in range(0,len(new_activityList)):
#                         RAmatrix[row][column] = return_list[i].iloc[count,column]
#                 count = count + 1
#         
#         
# #         snFullDF_final = pd.concat(return_list, ignore_index=True)
# #         matrix = snFullDF_final.values
#         
#         return RAmatrix
...
#     def manhattan_similarity(self,resource_1, resource_2):
#         sum_diff = 0
#         for index in range(len(resource_1)):
#             sum_diff = sum_diff + abs(resource_1[index] - resource_2[index])
#         
#         return 1 - sum_diff/len(resource_1)
